        ...
        <section className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <section className="mb-6 rounded-2xl border border-green/20 bg-linear-to-br from-green/10 via-white to-neutral-50 p-5">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p className="text-xs font-medium uppercase tracking-wide text-green">Օրվա մոտիվացիա</p>
                <h3 className="mt-1 text-xl font-semibold text-neutral-900">{motivationTitle(todayIncome, yesterdayIncome)}</h3>
                <p className="mt-2 max-w-2xl text-sm text-neutral-600">
                  {motivationMessage(todayIncome, yesterdayIncome, remainingToBeatYesterday)}
                </p>
              </div>
              <div className="rounded-xl border border-green/20 bg-white px-4 py-3 text-right shadow-sm">
                ...
                <p className="mt-1 text-2xl font-semibold text-green">{formatMoney(todayIncome)}</p>
                <p className="mt-1 text-xs text-neutral-500">
                  Ծախսերից հետո՝ <span className={todayFinalResult >= 0 ? 'text-green' : 'text-red-700'}>{formatMoney(todayFinalResult)}</span>
                </p>
              </div>
            </div>

            <div className="mt-5 grid gap-3 md:grid-cols-4">
              <MotivationMetric title="Երեկվա եկամուտ" value={formatMoney(yesterdayIncome)} />
              <MotivationMetric title="Երեկը անցնելու համար" value={remainingToBeatYesterday > 0 ? formatMoney(remainingToBeatYesterday) : 'Արդեն անցել ես'} />
              <MotivationMetric title="Այսօրվա գործարքներ" value={`${todayActionsCount} հատ`} />
              <MotivationMetric title="Երեկվա գործարքներ" value={`${yesterdayActionsCount} հատ`} />
            </div>
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              <MotivationMetric title="Այսօրվա կասայի ծախս" value={formatMoney(todayCashExpenseTotal)} />
              <MotivationMetric title="Օրվա վերջնական արդյունք" value={formatMoney(todayFinalResult)} />
            </div>
          ...
          <section className="mb-6 grid gap-4 lg:grid-cols-3">
            ...
            <InsightCard
              ...
              value={weekRecord.income > 0 ? formatMoney(weekRecord.income) : 'Դեռ ռեկորդ չկա'}
              text={
                weekRecord.income > 0
          ...
          <section className="mb-6 grid gap-4 lg:grid-cols-3">
            <div className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
              <p className="text-sm font-semibold text-neutral-900">Պարտքերի առողջություն</p>
              <p className="mt-2 text-sm text-neutral-600">
                Այսօր մարվել է <span className="font-semibold text-green">{formatMoney(todayDebtPaid)}</span>։
              </p>
              <p className={`mt-1 text-sm ${overdueDebts.length > 0 ? 'text-red-700' : 'text-neutral-600'}`}>
                Ժամկետանց պարտքեր՝ {overdueDebts.length} հատ ({formatMoney(overdueDebtAmount)})
              </p>
              <Link href={routes.dashboardDebts} className="mt-3 inline-block text-sm text-green hover:underline">
                Բացել պարտքերը →
              </Link>
            </div>

            <div className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
              <p className="text-sm font-semibold text-neutral-900">Պահեստի արագ ազդակ</p>
              {lowStockAccessories.length === 0 ? (
                <p className="mt-2 text-sm text-neutral-600">Քիչ մնացած ապրանք չկա։ Պահեստը առողջ է։</p>
              ) : (
                <div className="mt-2 space-y-1">
                  {lowStockAccessories.map((item) => (
                    <p key={item.id} className="text-sm text-neutral-600">
                      {item.name}՝ <span className="font-semibold text-red-700">{item.quantity} հատ</span>
                    </p>
                  ))}
                </div>
              )}
              <Link href={routes.dashboardAccessories} className="mt-3 inline-block text-sm text-green hover:underline">
                Բացել պահեստը →
              </Link>
            </div>

            <div className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
              <p className="text-sm font-semibold text-neutral-900">Օրվա խորհուրդ</p>
              <p className="mt-2 text-sm text-neutral-600">{dayTip}</p>
              {isSlowDay ? (
                <p className="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800">
                  Օրը մի քիչ դանդաղ է գնում․ փորձիր ակտիվացնել աքսեսուարների առաջարկը յուրաքանչյուր հաճախորդի հետ։
                </p>
              ) : null}
            </div>
          </section>
          ...
          <section className="mb-6 rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className="text-sm font-semibold text-neutral-900">Կասայի օրվա արդյունք</p>
                <p className="mt-1 text-sm text-neutral-600">
                  Եկամուտ՝ {formatMoney(todayIncome)} · Ծախս՝ {formatMoney(todayCashExpenseTotal)}
                </p>
              </div>
              <div className="text-right">
                <p className="text-xs text-neutral-500">Վերջնական արդյունք</p>
                <p className={`text-xl font-semibold ${todayFinalResult >= 0 ? 'text-green' : 'text-red-700'}`}>
                  {formatMoney(todayFinalResult)}
                </p>
                <Link href={routes.dashboardExpenses} className="mt-2 inline-block text-sm text-green hover:underline">
                  Գրանցել ծախս →
                </Link>
              </div>
            </div>
          ...
          <section className="mb-6 rounded-2xl border border-amber-200 bg-amber-50/60 p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className="text-xs font-medium uppercase tracking-wide text-amber-700">Պահեստի վերահսկում</p>
                <h3 className="mt-1 text-lg font-semibold text-neutral-900">Ավարտվող ապրանքներ</h3>
                <p className="mt-1 text-sm text-neutral-600">
                  Այստեղ երևում են այն ապրանքները, որոնց մնացորդը 5 հատ կամ պակաս է։
                </p>
              </div>
              <Link
            ...
            {lowStockAccessories.length === 0 ? (
              <p className="mt-4 rounded-xl border border-amber-100 bg-white px-4 py-3 text-sm text-neutral-600">
                Այս պահին ավարտվող ապրանքներ չկան։ Պահեստի մնացորդները նորմալ են։
              </p>
            ) : (
              <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                {lowStockAccessories.map((item) => (
                  <div key={item.id} className="rounded-xl border border-amber-100 bg-white p-3 shadow-sm">
                    ...
                    <div className="mt-3 flex items-center justify-between gap-2">
                      ...
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-semibold ${
                          item.quantity === 0 ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-800'
                        }`}
                      >
                        {item.quantity === 0 ? 'Սպառված' : `${item.quantity} հատ`}
                      </span>
                    </div>
                  </div>
                ))}
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            ...
            <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-4">
              ...
              <p className="mt-2 text-lg font-semibold text-neutral-900">{formatMoney(todayCashExpenseTotal)}</p>
              <Link href={routes.dashboardExpenses} className="mt-2 inline-block text-sm text-green hover:underline">
                Բացել էջը →
              </Link>
            </div>
