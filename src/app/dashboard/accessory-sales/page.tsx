      ...
      <div className="mx-auto max-w-6xl">
        <section className="space-y-5 rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
          <header className="border-b border-neutral-200 pb-4">
            <h2 className="text-2xl font-semibold text-neutral-900">Ակսեսուարի վաճառք</h2>
          ...
          <div className="rounded-xl border border-neutral-200">
            <div className="grid grid-cols-12 border-b border-neutral-200 bg-neutral-50 px-4 py-3 text-xs font-medium uppercase tracking-wide text-neutral-500">
              <p className="col-span-4">Ակսեսուար</p>
            ...
            <div className="divide-y divide-neutral-200">
              {sales.length === 0 ? (
                <p className="px-4 py-6 text-sm text-neutral-500">Առայժմ վաճառք չկա։</p>
              ) : (
                sales.map((item) => (
                  <div key={item.id} className="grid grid-cols-12 px-4 py-3 text-sm text-neutral-800">
                    <p className="col-span-4">{item.accessory.name}</p>
                    <p className="col-span-2">{item.quantity}</p>
                    <p className="col-span-2">{item.unitSalePrice.toString()}</p>
                    <p className="col-span-2">{item.totalSalePrice.toString()}</p>
